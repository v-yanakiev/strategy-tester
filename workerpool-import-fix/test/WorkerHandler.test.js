...
describe("WorkerHandler", function () {
  it("should handle a request", function (done) {
    ...
    handler.exec("run", [String(add), [2, 4]]).then(function (result) {
  ...
  it("should get all methods", function (done) {
    ...
    handler.methods().then(function (methods) {
  ...
  it("handle a promise based result", function (done) {
    ...
    handler.exec("run", [String(asyncAdd), [2, 4]]).then(function (result) {
      assert.strictEqual(result, 6);

      handler
        .exec("run", [String(asyncAdd), [2, "oops"]])
        .catch(function (err) {
          assert.ok(
            err.stack.match(/TypeError: Invalid input, two numbers expected/)
          );
  ...
  it("create a worker handler with custom script", function (done) {
    ...
    handler.exec("run", [String(add), [2, 4]]).then(function (result) {
      ...
      handler.exec("multiply", [2, 4]).then(function (result) {
  ...
  it("should handle the asynchronous initialization of a worker", function (done) {
    ...
    handler.exec("add", [2, 4]).then(function (result) {
      assert.strictEqual(result, 6);
  ...
  it("should handle errors thrown by a worker (1)", function (done) {
    ...
    handler.exec("run", [String(test)]).catch(function (err) {
  ...
  it("should handle errors thrown by a worker (2)", function (done) {
    ...
    handler.exec("run", [String(test)]).catch(function (err) {
      assert.ok(
        err.stack.match(/RangeError: Maximum call stack size exceeded/)
      );
  ...
  it("should handle crashing of a worker (2)", function (done) {
    ...
    handler
      ...
      .catch(function (err) {
        ...
        assert.ok(
          err.stack.match(/Error: Workerpool Worker terminated Unexpectedly/)
        );
  ...
  describe("tryRequireWorkerThreads", function () {
    it("gracefully requires or returns null", function () {
      ...
      assert(
        workerThreads === null || workerThreads === require("worker_threads")
      );
    });
  });
  ...
  describe("setupProcessWorker", function () {
    it("correctly configures a child_process", function () {
      ...
      assert.strictEqual(
        WorkerHandler._setupProcessWorker(
          SCRIPT,
          {
            forkArgs: FORK_ARGS,
            forkOpts: FORK_OPTS,
  ...
  describe("setupWorkerThreadWorker", function () {
    it("works", function () {
      ...
      var worker = WorkerHandler._setupWorkerThreadWorker(SCRIPT, {
        Worker: Worker,
      });
  ...
  describe("workerGracefulExit", function () {
    it("worker exit after master is killed", function (done) {
      var child = childProcess.fork(
        path.join(__dirname, "./forkToKill/common.js")
      );
